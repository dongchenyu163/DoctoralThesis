...
\begin{enumerate}
  ...
  \item \textbf{容器の形状モデリング}：液体注ぎタスクでは，注ぎ容器および目標容器の内部形状を点群モデルとして取得し，容積計算することおよび液面高さから注いた量の計算するために用いる．
...
卓上スキャンのみで得られる点群は，上面および側面にのみを観測であり，底面側の点，法線，および外形の完全性が欠落する．
そのため，食材や不定形物体の外形モデルを用いた把持候補生成，幾何学的重心の近似，および外形再構成精度の評価では，観測方向に依存した偏りが生じる可能性がある．
したがって，本節では，卓上スキャンでは観測できない底面側の外表面を補完するため，対象物を把持して作業台から離し，底面側を観測可能な状態に変換した上で点群を取得する手法を述べる．

本節では，双腕ロボットの把持動作を能動的な視点生成手段として用い，卓上観測では取得できない底面点群を取得する手法を述べる．
...
\begin{table}[p]
  ...
  \begin{tabular}{llrrrrrrr}
    \toprule
    \textbf{Distance [mm]} & \textbf{Stat.} & \textbf{Radius err. [mm]} & \multicolumn{3}{c}{\textbf{Axis direction err. [-]}} & \multicolumn{3}{c}{\textbf{Top-center err. [mm]}} \\
...
対象物は，3Dプリンタで作製した5種類の容器であり，それぞれStraightCup，RainDrop，SquaredCup，SpheredCup，RamppedCupと呼ぶ．
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrrrr}
    ...
    \midrule
    No.1 (StraightCup) & 575.48 & $569.92\pm0.97$ & 574.10 &  4.18 & 0.73 \\
    No.2 (RainDrop)   & 366.13 & $362.00\pm0.55$ & 374.48 & 12.48 & 3.45 \\
    No.3 (SquaredCup) & 194.70 & $193.38\pm0.56$ & 184.40 & -8.98 & 4.65 \\
    No.4 (SpheredCup) & 221.05 & $218.92\pm0.39$ & 217.88 & -1.04 & 0.47 \\
    No.5 (RamppedCup) & 140.83 & $141.63\pm0.70$ & 138.45 & -3.18 & 2.25 \\
    \bottomrule
  \end{tabular}
\end{table}

5種類の容器に対する平均絶対誤差は5.97\,$\mathrm{cm}^3$，平均相対誤差は2.31\%であった．
No.1およびNo.4では相対誤差が1\%未満であり，提案した点群取得と断面積分により，単純な円筒形状だけでなく曲面を含む容器でも高い精度で容積を推定できることが分かった．
一方，No.2では推定値が真値より12.48\,$\mathrm{cm}^3$大きく，No.3では8.98\,$\mathrm{cm}^3$小さかった．
...
No.5のように開口部や側面に傾斜を含む容器では，スライスの上端判定と縁付近の点群欠損が体積誤差に影響する．
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrr}
    ...
    \midrule
    No.1 (StraightCup) & 1.952 & 2.931 & 3.522 \\
    No.2 (RainDrop)   & 1.066 & 0.945 & 1.424 \\
    No.3 (SquaredCup) & 1.929 & 1.465 & 2.422 \\
    No.4 (SpheredCup) & 1.482 & 1.201 & 1.907 \\
    No.5 (RamppedCup) & 2.723 & 1.954 & 3.352 \\
    \bottomrule
  \end{tabular}
\end{table}
...
最も小さい誤差を示したNo.2では，平均誤差が1.066\,mm，RMSEが1.424\,mmであり，多視点合成とMLS平滑化が有効に働いた．
一方，No.1およびNo.5ではRMSEが3\,mmを超えた．
No.1の誤差が大きくなった原因としては，容器サイズが大きく，視点間の重複領域における微小な位置合わせ誤差が全体に蓄積したことが考えられる．
また，No.5は傾斜面を含むため，視線入射角が大きい領域で深度欠損が発生しやすく，局所的な点群密度低下がRMSEを増加させたと考えられる．
...
\begin{table}[t]
  \centering
  \caption{Distance errors of point clouds including bottom-surface acquisition.}
  ...
  \begin{tabular}{lrrr}
    ...
    \textbf{Object} & \textbf{Avg. [mm]} & \textbf{Std. [mm]} & \textbf{RMSE [mm]} \\
    \midrule
    No.1 & 4.769 & 3.056 & 5.715 \\
    No.2 & 4.890 & 3.567 & 6.091 \\
    No.3 & 5.095 & 4.446 & 6.768 \\
    No.4 & 5.238 & 3.419 & 6.327 \\
    No.5 & 4.910 & 4.673 & 6.789 \\
    \bottomrule
  \end{tabular}
\end{table}

5物体の平均距離誤差は4.98\,mm，平均RMSEは6.34\,mmであった．
物体間で平均誤差の差は小さく，最小値はNo.1の4.769\,mm，最大値はNo.4の5.238\,mmであった．
...
この情報は，物体の幾何学的重心の近似，把持候補点の生成，および切断時把持における接触可能領域の評価に有効である．
...
これらの結果は，本章で構築した点群取得・処理パイプラインが，後続章の皮むき，注ぎ，および切断時把持に必要な幾何情報を提供できることを示している．
