...
手先に固定したカメラを用いる構成では，手先座標系 $\Sigma_H$ に対するカメラ座標系 $\Sigma_C$ の固定変換 ${}^{H}\bm{T}_{C}$ をハンドアイキャリブレーションにより求めた．
...
DENSO VS-050 （詳細は構成変更の\ref{sec:system:configuration-changes:hardware}節）では，ロボットの可動範囲の制約を補うため，RGB-D カメラを1軸サーボアームに取り付けた（図\ref{fig:system:robot_camera_arm}）．
...
以上のキャリブレーションにより，固定カメラ構成および1軸サーボカメラアーム構成のいずれにおいても，RGB-D カメラで取得した点群を世界座標系へ変換し，多視点点群として合成できる．
