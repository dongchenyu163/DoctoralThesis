...
一つは，三菱重工製PA10-7Cを二台用いた双腕ロボットシステムであり，注ぐ動作，皮むき動作，および切断時把持の検証に用いた．
もう一つは，DENSO製VS-050を二台用いた双腕ロボットシステムであり，切断時把持の実機検証に用いた．
...
および各タスクにおける工具先端位置を同一の基準で扱うため，複数の座標系を定義する．
...
\begin{enumerate}
  ...
  取得した点群，操作対象物，工具先端位置，および左右腕の動作目標は，
  最終的にこの世界座標系上で記述する．
  ...
  各タスクで使用する工具やエンドエフェクタの操作を容易にするため，
...
取り付け関係として決定される．
また，ツール座標系は，各タスクの工具やエンドエフェクタに応じてユーザが定義し，
手先座標系に対する固定変換として扱う．
...
実際に作業を行う工具先端を目標位置・姿勢へ直接誘導できる．
例えば，ピーラーでは刃先位置，注ぎ作業では容器把持部または容器姿勢，
切断時把持では指先接触点や包丁の刃先位置を基準として動作を記述できる．
...
DENSO VS-050を用いたロボットシステムでは，ロボットの可動範囲の制約を補うため，RGB-Dカメラを1軸サーボアームに取り付けた（図\ref{fig:system:robot-camera-arm-denso}）．
