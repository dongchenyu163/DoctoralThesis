...
VS-050は6軸の多関節ロボットであり，可搬質量4\,kg，位置繰り返し精度$\pm 0.02$\,mmの性能を有する．PA10-7Cと同様に双腕構成で使用し，双腕のベース座標系間の距離（図\ref{fig:robot_env}の右側のロボットを参照する）は$XY$軸方向にそれぞれ777\,mm -405\,mmとした．ツール座標系の定義や世界座標系（左アームのベース座標系に設定）の設定はPA10-7Cの場合と同様である．
...
DENSO VS-050においても同様に，手先フランジ中心を基準としたツール座標系を定義し，ツール先端の位置姿勢を制御の基準とした．
...
\todofix[
ただし，第三（把持）研究後半のロボット交換の可動範囲の減少の影響で，カメラの固定は1軸のサーボアーム（図\ref{fig:robot_camera_arm}）を利用し，動けるようになった．
\begin{figure}[ht]
    \centering
    \includegraphics[width=.7\linewidth]{fig/chp3/chp3_new_robot_camera_arm.png}
    \caption{The camera arm for DENSO VS-050. }
    \label{fig:robot_camera_arm}
\end{figure}
]
...
\begin{figure}[t]
  ...
  \caption{The pipeline of point cloud processing}
  \label{fig:pipeline}
\end{figure}
...
\paragraph{位置姿勢と範囲}
\label{par:dfdfdf}
点群モデルのAABBや主成分を計算することにより，世界座標系における物体の位置姿勢や範囲を取得する．この情報は，ロボットアームの到達可能性判定，衝突回避，およびカメラの再撮影時の視点計画に利用される．\ref{par:dfdfdf}
...
% \begin{figure}[t]
%   \centering
%   \includegraphics[width=0.75\linewidth]{fig/chp3/pointalgo_cross_section.png}
...
\begin{enumerate}
  ...
  位置合わせ後，式(\ref{eq:full_model})により両者を合成し，底面を含む完全な物体モデルを生成する．
\end{enumerate}
