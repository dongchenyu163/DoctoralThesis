...
$\Omega_{\mathrm{c1,2}}$ & & Contact surfaces (right part in Fig.\ref{fig:method}) \\
...
\State
\For{\textbf{all} $(T, \bm v) \in \mathbb{T}$}  \label{algoline:global-knifepose-for-st}

    \State $\Omega_{\mathrm{c1}}, \Omega_{\mathrm{c2}}, \Omega_{\mathrm{g}}, \equsearchspace_\mathrm{init} \gets $\Call {PrepareData}{$\Omega$, $T$}  \label{algoline:exp1-st}    \label{algoline:global-PrepareData}
    \State $\equsearchspace \gets $\Call {FilterByGeoScore}{$\equsearchspace_\mathrm{init}$, $T$}

    \For{$P \in (\mathbb{P}_\mathrm{all} \setminus \equsearchspace)$  }  \label{algoline:global-set-inf-score-st}
        \State $\mathbb{S}[\equholdingpointset] \gets -\infty$
    \EndFor  \label{algoline:global-set-inf-score-ed}
  
    \State $\{S_\mathrm{pos, i}\} \gets$ \Call {CalPositionalScore}{$\equsearchspace$, $T$} 

    \State $\bm{t}_\mathrm{k} \gets $\Call {CalKnifeForce}{$\Omega_{\mathrm{c1}}$, $\Omega_{\mathrm{c2}}$, $T$, $\bm v$}  \label{algoline:global-CalKnifeForce} \label{algoline:global-filter-force-st}

    \State $\{S_\mathrm{force, i}\} \gets$ \Call {CalDynamicScore}{$\bm{t}_\mathrm{k}, \equsearchspace $, $\Omega_{\mathrm{g}}$} \label{algoline:global-caldynamicscore}
  
    \For{$i\gets 1, N$} \label{algoline:global-accumulate-forcescore-st}
        \State $ \mathbb{S}[\equholdingpointset[][i]] \gets \mathbb{S}[\equholdingpointset[][i]] + S_{\mathrm{pos}, i} + S_{\mathrm{force}, i}$
    \EndFor \label{algoline:global-point-for-ed} \label{algoline:global-filter-force-ed}  \label{algoline:global-accumulate-forcescore-ed}
  
\EndFor  \label{algoline:global-knifepose-for-ed}
...
\EndFunction\\
\Function{Normalize}{$\{E_i\}$}  \label{algoline:normalize-func}
...
\EndFunction  \label{algoline:normalize-func-ed}
\end{algorithmic}
\end{algorithm}
...
\Function{CalPositionalScore}{$\mathbb{P}$, $T$}
    ...
    \For{$i\gets 1, N$}
        ...
        \State $E_{\mathrm{pdis,}i}\gets E_\mathrm{pdis}$ for $P_i$ \label{algoline:CalDynamicScore-pos-score}
    \EndFor
    \State $\{E_{\mathrm{pdir,}i}\}\gets$ \Call{Normalize}{$\{E_{\mathrm{pdir,}i}\}$} \label{algoline:score-pos-final-st}
    ...
    \EndFor  \label{algoline:score-pos-final-ed}
...
\Function{CalDynamicScore}{$\bm{t}_\mathrm{k}$, $\equsearchspace$, $\Omega_{\mathrm{g}}$}
    ...
    \State \fix[$\{\tilde{E}_{\mathrm{mag,}i}\}\gets$ \Call{Normalize}{$\{E_{\mathrm{mag,}i}\}$} \label{algo:force-global-norm-score-st} ]
    \State \fix[$\{\tilde{E}_{\mathrm{dir,}i}\}\gets$ \Call{Normalize}{$\{E_{\mathrm{dir,}i}\}$}]
    \State \fix[$\{\tilde{E}_{\mathrm{var,}i}\}\gets$ \Call{Normalize}{$\{E_{\mathrm{var,}i}\}$}] \label{algo:force-global-norm-score-ed}
    ...
    \State \fix[\Return $\{S_{\mathrm{dyn,}i}\}$ \label{algoline:CalDynamicScore-all}]
\EndFunction
