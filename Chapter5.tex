\chapter{アプローチの検証：液体注ぐ作業}
\label{ch:pour}
% ============================================================================

本章では，第\ref{ch:introduction}章で提案した「3Dモデルに基づく分析主導型アプローチ」を，
料理作業において高い頻度で現れる「注ぐ」動作に適用する．
注ぎ作業は，直接把持できない液体を対象とし，容器という環境を介して間接的に操作する点に
特徴がある．したがって，本作業は第\ref{ch:introduction}章\ref{tab:tasks}表に示した
「容器間関係（環境的分析）」と「容積・形状（幾何学的分析）」の検証に位置づけられる．

本章では，第\ref{ch:system}章で構築した容器の3D点群モデルを基盤とし，
幾何学的分析による注いだ体積の推定と，動作制御による精密な注ぎ実行を統合することで，
未知形状の容器に対しても外部計測道具を用いずに定量的な注ぎを実現する2つの手法を提案する．

本章の構成は以下の通りである．
まず\ref{sec:pour_problem_overview}節で注ぎ作業の問題設定と提案手法の全体像を述べる．
次に\ref{sec:pour_analysis}節では，本手法のコアとなる幾何学的分析として，
...
作業時間が増大する．
ロボットによる注ぎの場合，カメラ等のセンサを用いて容器形状を計測し，
注いだ液体の体積を計算することが可能であるため，
計量道具を用いる手間を省くことができる．
...
\begin{description}
  \item[課題1] 注いだ体積の測定．
    ...
    容器の形状や姿勢にできる限り依存しない，汎用性の高い方法が求められる．
  \item[課題2] 注ぎ動作の制御．
...
（図\ref{fig:pour_methods_overview}）．
% ここではまだ具体的な図はないが，後の実験で示す容器の図を先に配置することも可能．
% この見出しに対応する図が IROS2019 にはないため，必要に応じて追加．
...
% ---------------------------------------------------------------------------
\subsection{液面検出による注いだ体積のリアルタイム推定}
...
注ぎが早期に停止する現象が確認された．
容器3では平均誤差-0.95\,ml，標準偏差1.16\,mlと非常に高い精度を達成し，
家庭用調理に十分な性能を有することが実証された．
...
\begin{figure*}[t]
	...
	\begin{subfigure}[t]{.45\textwidth}
		...
		\label{fig:method-exp1.sub2}
	\end{subfigure}
	\caption{Influence of Surface Tension on Method 2. In Figure \ref{fig:method-exp1.sub1}, the blue, orange, and greeb lines show the results of Cup~2 with initial volumes of 30, 60, and 90\,ml, respectively. Cup~3 is 22, 44, and 66\,ml (Figure \ref{fig:method-exp1.sub2}). The purple line shows the result with no surface tension and Cup~2 with an initial volume of 90\,ml.}
	\label{fig:method-exp1}
\end{figure*}
% ------------------------------------------------------------------------------------
...
\textbf{考察：}
表面張力の有無による比較実験から，表面張力が注いだ量推定に与える影響が定量的に明らかとなった．
...
%     \midrule
%     制御方式 & PD制御（注いだ量フィードバック） & P制御（角度追従） \\
%     \midrule
...
\begin{enumerate}
  ...
  手法1の誤差は，事前学習や事前モデルを前提とする先行研究（約 $38\,\mathrm{ml}$ の誤差\cite{Related1}，約 $5\,\mathrm{ml}$ の誤差\cite{Related2}）と比較しても，家庭での調理に十分な水準にある．  
  \item \textbf{実装要件：}  
  ...
  \item \textbf{両手法の使い分け：}  
  高い定量性が要求される調理（調味料の計量など）には手法1が適している．  
  カメラ配置の自由度が低い環境や，大量の液体を注ぐため表面張力の相対的な影響が小さい場合には，手法2が有効であると考えられる．
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{分析結果に基づく動作制御の実装：}
    幾何学的分析の結果を動作制御へ接続するため，
    手法1（注いだ体積フィードバックPD制御＋戻し動作）と
    手法2（目標角度追従P制御＋残留液流出待機＋戻し動作）
    の2つの制御手法を設計・実装した．
  \item \textbf{実機実験による検証：}
    ...
    これにより，外部計測道具を用いずに，未知形状の容器に対しても定量的な注ぎが可能であることを実証した．
    特に手法1は，先行研究と比較して高い精度を示し，家庭用調理に十分な性能を有することが確認された．
\end{enumerate}

本章の結果は，第\ref{ch:introduction}章\ref{tab:tasks}表に示した
検証作業のうち，「容器という環境を利用した流体操作」に対応するものであり，
環境的・幾何学的分析の統合により，直接把持できない液体対象に対しても
定量的な操作が可能であることを示した．
次章では，形状と環境の分析を中心とした本章から，包丁外力に抵抗する把持安定性の分析へ進む．食材形状，包丁軌道を同時に用い，力学的・環境的分析から把持位置を決定する．
